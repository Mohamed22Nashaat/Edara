...
/* Insert*/
        await query('INSERT INTO user SET ? ',userInfo);
...
router.put("/:id",
        ...
        try{
            const query = util.promisify(conn.query).bind(conn);
            let user = await query("SELECT * FROM user WHERE id = ?", req.params.id);
            ...
            user = await query("SELECT * FROM user WHERE id = ?", req.params.id);
            if(req.body.status == 'inactive'){
                await query('UPDATE warehouses SET supervisorID = NULL WHERE supervisorID = ?', user[0].id);
                await query('UPDATE user SET warehouseID = NULL WHERE id = ?', user[0].id);
            }
            user = await query("SELECT * FROM user WHERE id = ?", req.params.id);
            const userInfo = {
            ...
            await query('UPDATE user SET ?  WHERE id = ?',[userInfo, user[0].id]);
...
router.get("/",
    ...
    try{
        const query = util.promisify(conn.query).bind(conn);
        const user = await query("SELECT * FROM user WHERE role != 'admin'");
