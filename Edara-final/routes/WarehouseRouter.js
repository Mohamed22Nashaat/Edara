...
router.put("/:id",
    ...
    try{
        ...
        if(req.body.supervisorID){
            const checkUser = await query('SELECT * FROM user WHERE id = ?', req.body.supervisorID);
            ...
            await query('UPDATE user SET warehouseID = NULL WHERE warehouseID = ?', warehouse[0].id);
            await query('UPDATE user SET warehouseID = ? WHERE id = ?', [warehouse[0].id, req.body.supervisorID]);
        }
