...
function UpdateSupervisore  () {
  const {id} = useParams();
  useEffect(()=>{
    axios.get('http://localhost:4000/UpdateSupervisore/'+id)
    .then(res => {
      console.log(res)
...
const submit =(event)=>{
  ...
  axios.put('http://localhost:4000/UpdateSupervisore/'+id,values )
        .then(res => {
            console.log(res);
            navigate('/homess')
         //.catch(err => console.log(err))
...
const handleInputChange =(event)=>{
  ...
  event.preventDefault();
  axios.delete('http://localhost:4000/users/'+id,values )
        .then(res => {
            console.log(res);
            navigate('/homess')
         //.catch(err => console.log(err))
